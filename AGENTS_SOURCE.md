# AGENTS

## Scope

This repository is the canonical source for one developer's reusable AI assets.

## MUST

- Read `governance/constitution.md` and `governance/AIBEHAVIOR.md` before meaningful changes.
- Read `governance/ponytail-agents.md` when using Ponytail assets.
- Read `governance/impeccable-agents.md` when using Impeccable assets.
- Treat `governance/`, `skills/`, `agents/`, and `wiki/` as canonical shared roots.
- Place generated text and files under `artifacts/`; use `artifacts/tickets/` for local tickets.
- Search `wiki/` first for durable project context.
- Keep the wiki small and update it when shared assets, rules, or structure change.
- Use `apply_patch` for manual edits.
- Preserve unrelated user changes.
- Verify relevant behavior before reporting completion.

## DO NOT

- Maintain copied forks in tool-specific homes.
- Refactor unrelated legacy code.
- Use destructive Git commands unless explicitly requested.
- Prefer repository-local shared roots; use linked user-root assets only as fallback.
