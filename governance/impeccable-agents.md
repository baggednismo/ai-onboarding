...
- Write retained Impeccable reports, audits, screenshots, exports, logs, and
  scratch notes under `artifacts/impeccable/`; use `artifacts/tickets/` for
  tickets.
- Do not write generated vendor output into `skills/`, `governance/`,
  `hooks/`, `agents/`, or `wiki/`. Those are canonical source roots.
- Do not recreate the upstream `dist/`, `build/`, `site/`, or provider-output
  trees in this repository. They are not part of the vendored runtime.
- `PRODUCT.md`, `DESIGN.md`, and `.impeccable/` are intentional Impeccable
  project context and runtime state. Keep them at the project paths the skill
  expects when the user explicitly initializes or uses those features; they
  are not report artifacts and should not be moved into `artifacts/`.
- Source edits belong in the user's project source tree. Only the resulting
  durable reports and generated supporting files belong under the artifact
  policy above.
